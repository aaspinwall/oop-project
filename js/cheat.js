...
const items = [
  {
    ...
    value: 5,
    rarity: 0,
    use: target => usePotion(target, 25),
  },
  {
    name: RARITY_LIST[1] + " potion",
    type: "item",
    value: 10,
    rarity: 1,
    use: target => usePotion(target, 70),
  },
  {
    name: RARITY_LIST[2] + " potion",
    type: "item",
    value: 20,
    rarity: 2,
    use: target => usePotion(target, 200),
  },
  {
    name: RARITY_LIST[3] + " potion",
    type: "item",
    value: 50,
    rarity: 3,
    use: target => usePotion(target, target.getMaxHp()),
  },
  {
    ...
    value: 7,
    rarity: 0,
    use: target => useBomb(target, 50),
  },
  {
    name: RARITY_LIST[1] + " bomb",
    type: "item",
    value: 12,
    rarity: 1,
    use: target => useBomb(target, 120),
  },
  {
    name: RARITY_LIST[2] + " bomb",
    type: "item",
    value: 25,
    rarity: 2,
    use: target => useBomb(target, 300),
  },
  {
    name: RARITY_LIST[3] + " bomb",
    type: "item",
    value: 100,
    rarity: 3,
    use: target => useBomb(target, target.hp * 0.9),
  },
  {
  ...
  },
  {
    ...
    use: function(target) {
      ...
      target.name = target.name
        ...
        .join("");
      print(
        `${target.name}, target is confused and hurts itself in the process`,
        "red"
      );
...
function updatePlayerPosition(newPos) {
  ...
  }; // Clear the player at current position
...
function hit(target, val) {
  ...
  print(
    `${target.name} hit! -${val}hp`,
    target.type === "player" ? "red" : "purple"
  );
  ...
  if (target.hp <= 0) {
    ...
    if (target.type === "player") gameOver();
      ...
      if (exp >= player.getExpToLevel()) {
        ...
        player.exp = exp;
        print(
          `Congratulations! You have received ${target.getExp()} exp points.`
        );
      }
      if (target.items.length > 0) {
        ...
        if (target.items.some(item => item.name.includes("key"))) {
          print(
            "You got the key! You can go to the dungeon and free the princess :)"
          );
        }
      }
    }
...
function startTrade(tradesman) {
  print(
    `Encountered ${
      tradesman.name
    }! You can buy(itemIdx) and sell(itemIdx) items $$$`
  );
...
function openDungeon(dungeon) {
  ...
  if (dungeon.hasPrincess) {
    ...
    print(
      `As consolation, you found ${dungeon.items.length} items and ${
        dungeon.gold
      } gold.`
    );
    print(dungeon.items);
...
function encounterDungeon(dungeon) {
  ...
  if (dungeon.isLocked) {
    ...
    print(
      "Rumours are some monsters have keys to dungeons. The tradesman might also have spare keys to sell but they don't come cheap"
    );
  } else {
    openDungeon(dungeon);
  }
...
function setupBoard() {
  ...
  updateBoard(
    createDungeon({ row: 2, column: 4 }, false, false, [items[0], items[1]], 40)
  );
  next();
}
